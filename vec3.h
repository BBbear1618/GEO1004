...
inline vec3 operator*(float scalar, const vec3& v) {
    ...
    return vec3(scalar * v(0), scalar * v(1), scalar * v(2)); // replace this line
}
...
inline float vec3::dot(const vec3& other) const {
	...
	return m_data[0] * other(0) + m_data[1] * other(1) + m_data[2] * other(2); // replace this line.
}
...
inline float& vec3::operator()(int n) {
	assert("vec3::operator() -- invalid argument" && n < 3);
	return m_data[n];
}


inline float vec3::operator()(int n) const {
	assert("vec3::operator() const -- invalid argument" && n < 3);
	return m_data[n];
}
